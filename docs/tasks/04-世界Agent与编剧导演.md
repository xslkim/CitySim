...
> v1.4 · 2026-09-25（M3 实施收口，非评审轮）：T-WA-01~10 / T-DIR-01~06 全部落地；新增偏差 D-22~D-33（节假日实历日期/illness 天数档/模板池/部分补扣足额才扣/manager_id 空值/职场时点/exit 优先于 if_blocked/L1 标记载体/grade_revise interventions.level=D-30 L2/promotion_window 不停发+退租危机映射/mock K3 桩/smoke m3 段接收）；D-04 细化（day_summary 时点=翻界前最后 tick 格、day=刚结束日序号，replay 窗口相容）；上游接线补网：Grader 阈值配置化（02 T-ADJ-07 实现不动，阈值段注入）、validators/dialogue R2/R3 阈同源、聚合器/时钟/rotation/reflector/grade_revise 补 ui.grade 全覆盖、审计⑥ SQL 增 WITH ORDINALITY（事件内 changes 保序，M3 实测回归）。
...
| D-04 | `time.day_summary` 触发时点设计未定义 | 工程默认：sim 日期翻转时恰一条；`payload.day`=**刚结束日**的自纪元模拟日序号；sim_time 取翻界前最后 tick 格（23:55 档，M3 实测细化：保证「恰为第 N 模拟日」实跑的重放对账窗口相容，04 §5.3/02 T-ADJ-08）；生产方=world_agent 日历日界钩子（06 §1.2 v1.2 注）；已登记（本文偏差表） |
