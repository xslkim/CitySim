...
| D42 | **撞墙条件③ p95 延迟阈条目级覆盖**（`p95_trip_ms`：glm-4.5-flash=60000、glm-4-flash=15000） | 04 §8.2 条件③ p95>8s 为设计口径；免费档 reasoning 模型固有延迟实测 15~25s（2026-09-25），按 8s 会全程误 trip → 条目级阈，付费档/上线期恢复 8s 口径 | **已登记（本文偏差表）** |
