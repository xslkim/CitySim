...
| D40 | **glm-4.5-flash 行 `max_tokens` 实测放大**（star_decision 400→2000、dialogue 1200→6000、reflection 500→3000、director 2000→8000） | reasoning_content 消耗 completion 配额（00 §1 A9；2026-09-25 实测 max_tokens=400 时 finish=length 空正文，2000 时正常 ~667 completion tokens）；04 §8.6 为设计镜像（`test_models_config.py` 断言 ≥ 设计值），glm-4-flash（非 reasoning）行不动 | **已登记（本文偏差表）** |
| D41 | **限流参数落模型条目级**（`providers.zhipu.chat[].rpm_limit/concurrency`）+ 网关增在飞并发闸 | 04 §8.5 桶口径为 provider 级；实测（2026-09-25）glm-4.5-flash 免费档瓶颈是**并发上限≈2**（超出即 429）而非 RPM，两型号限额不同 → 条目级配置 + `RateLimitedClient` 信号量；网关客户端键 = (provider, model) | **已登记（本文偏差表）** |
| D42 | **撞墙条件②③免费档条目级停用**（`p95_trip_ms: 0` / `rpm_usage_trip_pct: 0` = 停用；glm-4.5-flash/glm-4-flash 均停用，条件①连续 429/5xx 仍生效） | 04 §8.2 条件②（窗口 RPM>90%）/③（p95>8s）为设计口径；实测（2026-09-25 三次真跑）：免费档桶容量 = 提供方实测上限，打满桶是限流器本职非提供方撞墙（条件②恒误 trip）；延迟 2~12s 起步、服务端排队抖动 p95 可冲 115s（条件③条目级阈无论 15s/30s/60s 均误伤）→ 开发期两条目级停用， paced 生产/付费档恢复 90%/8s 口径；机制（条目级覆盖/窗口/p95 计算）保留并有单测 | **已登记（本文偏差表）** |
...
| D46 | **T-LLM-12 验收 1 口径适配**：原判据 `COUNT(*)=COUNT(*) FILTER (provider='zhipu')` 写作时 embed 预设走智谱；A10 定案本地 bge-m3 后 embed 行 provider='local_embed' 为合法真实调用 → 判据调整为：全量 >0 且 zhipu >0 且 **mock = 0**（干净库复测实测 2125/790/0，2026-09-25） | 00 §1 A10 定案的必然推论；验收语义（全量且真实、无 mock 混入）不变 | **已登记（本文偏差表）** |
